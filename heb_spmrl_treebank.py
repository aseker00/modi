...
def _process_treebank(tb_root_dir_path, tb_type):
    ...
    if removed_idx:
        ...
        lattices = [l for i, l in enumerate(lattices) if i not in removed_idx]
    lattices_df = _transform_lattices_to_dataframe(lattices, tokens)
    gold_lattices_df = _transform_lattices_to_dataframe(gold_lattices, tokens)
    return lattices_df, gold_lattices_df
...
def _load_dataframe(data_dir_path, partition_type, lattices_type):
    ...
    return pd.read_csv(str(csv_file_path), index_col=0)


# def load_dataframe(data_dir, partition):
#     res = {}
#     for partition_type in partition:
#         lattices_df = _load_dataframe(data_dir, partition_type, 'lattices-with-type')
#         gold_lattices_df = _load_dataframe(data_dir, partition_type, 'gold-lattices-with-type')
#         res[partition_type] = (lattices_df, gold_lattices_df)
#     return res
...
def load_lattices(root_dir_path, partition):
    res = {}
    data_dir = root_dir_path / 'treebank/spmrl/heb'
    for partition_type in partition:
        lattices_df = _load_dataframe(data_dir, partition_type, 'lattices-with-type')
        gold_lattices_df = _load_dataframe(data_dir, partition_type, 'gold-lattices-with-type')
        lattices = dataframe_to_lattices(lattices_df)
        gold_lattices = dataframe_to_lattices(gold_lattices_df)
        res[partition_type] = (lattices, gold_lattices)
    return res
    # parition_dataframes = load_dataframe(root_dir_path / 'treebank/spmrl/heb/data', partition)
...
def main():
    ...
    for partition_type in partition:
        lattices_df, gold_lattices_df = _process_treebank(tb_root_dir_path, partition_type)
        lattices_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-lattices.csv'
        gold_lattices_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-gold-lattices.csv'
        lattices_df.to_csv(str(lattices_file_path))
        gold_lattices_df.to_csv(str(gold_lattices_file_path))
    for partition_type in partition:
        lattices_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-lattices.csv'
        lattices_with_type_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-lattices-with-type.csv'
        lattices_df = pd.read_csv(lattices_file_path, index_col=0)
        lattices_df = _add_morpheme_type(lattices_df)
        lattices_df.to_csv(lattices_with_type_file_path)
        gold_lattices_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-gold-lattices.csv'
        gold_lattices_with_type_file_path = root_dir_path / 'treebank/spmrl/heb' / f'{partition_type}-gold-lattices-with-type.csv'
        gold_lattices_df = pd.read_csv(gold_lattices_file_path, index_col=0)
        gold_lattices_df = _add_morpheme_type(gold_lattices_df)
        gold_lattices_df.to_csv(gold_lattices_with_type_file_path)
