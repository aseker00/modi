...
def get_analysis(morphemes):
    ...
    for morpheme in morphemes:
        ...
        morpheme_type = morpheme[2]
        # morpheme_from_node_id = int(morpheme[3])
        # morpheme_to_node_id = int(morpheme[4])
        morpheme_form = morpheme[5]
        morpheme_lemma = morpheme[6]
        morpheme_tag = morpheme[7]
        morpheme_feats = morpheme[8]
        m = (morpheme_form, morpheme_lemma, morpheme_tag, morpheme_feats)
        analysis[morpheme_type].append(m)
    return analysis


def _get_raw_token_samples(partition):
    raw_samples = defaultdict(list)
    for partition_type in partition:
        lattices = partition[partition_type]
        for sent_id in lattices:
            raw_sent_sample = []
            for sent_token_id in lattices[sent_id]:
                token_morphemes = next(iter(lattices[sent_id][sent_token_id].values()))
                token = token_morphemes[0][9]
            ...
            raw_samples[partition_type].append(raw_sent_sample)
    return raw_samples


def _get_raw_morpheme_samples(partition):
    raw_samples = defaultdict(list)
    for partition_type in partition:
        lattices = partition[partition_type]
        for sent_id in lattices:
...
def _save_token_dataset(root_dir_path, partition):
    raw_token_samples = _get_raw_token_samples(partition)
    token_vocab = _extract_vocab(raw_token_samples)
...
def load_token_dataset(root_dir_path, partition):
    ...
    token_data_arrays = _load_data_arrays(token_vocab, raw_token_samples)
    token_dataset = _load_tensor_dataset(token_data_arrays)
    return token_vocab, token_dataset


def _save_morpheme_dataset(root_dir_path, partition):
    raw_morpheme_samples = _get_raw_morpheme_samples(partition)
    morpheme_vocab = _extract_vocab(raw_morpheme_samples)
...
def load_morpheme_dataset(root_dir_path, partition):
    ...
    morpheme_data_arrays = _load_data_arrays(morpheme_vocab, raw_morpheme_samples)
    morpheme_dataset = _load_tensor_dataset(morpheme_data_arrays)
    return morpheme_vocab, morpheme_dataset
