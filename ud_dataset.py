...
def to_token_lattice(token_lattice_ids, data_vocab):
...
def to_data_vocab(tokens, chars, forms, lemmas, tags, feats):
...
def get_data_vocab(lattices_dataset):
    ...
    return to_data_vocab(tokens, chars, forms, lemmas, tags, feats)


def get_data_vocabs_union(dv1, dv2):
...
def save_data_vocab(data_vocab_dir_path, data_vocab):
...
def load_data_vocab(data_vocab_dir_path):
    data_vocab = {}
...
def load_data_ft_vec(root_path, ft_root_path, data_vocab, la_name):
...
def to_data_tokens_row(lattice_data_row, data_vocab, token_char_ids):
...
def get_lattice_data_row_values(lattice_data_row, max_num_feats, data_vocab, infuse):
...
def get_data_row_values(data_row, data_vocab):
...
def _get_token_samples(lattices_df, data_vocab):
    ...
    data_token_rows = [to_data_tokens_row(lattice_data_row, data_vocab, token_char_ids)
                       for lattice_data_row in lattices_df.itertuples()]
    tokens_samples_df = pd.DataFrame([token_row for sent_token_rows in data_token_rows
...
def _get_fixed_analysis_samples(analyses_df, data_vocab, max_morphemes):
    ...
    analysis_values = [get_data_row_values(data_row, data_vocab) for data_row in analyses_df.itertuples()]
    analysis_samples_df = pd.DataFrame(analysis_values, columns=column_names)
...
def _get_var_morpheme_samples(analyses_df, data_vocab, max_morphemes):
    ...
    morph_column_names = ['form_id', 'lemma_id', 'tag_id', 'feats_id']
    column_names += morph_column_names
    morpheme_values = [get_data_row_values(data_row, data_vocab) for data_row in analyses_df.itertuples()]
...
def load_gold_data_samples(root_path, partition, la_name, tb_name):
    ...
    data_vocab = load_data_vocab(root_path / root_path / la_name / tb_name / 'vocab')
    ...
    morpheme_samples = {t: _get_var_morpheme_samples(gold_dataset[t], data_vocab, max_morphemes[partition[-1]])
                        for t in gold_dataset}
    return token_samples, morpheme_samples, data_vocab


def load_gold_super_data_samples(root_path, partition, la_name, tb_name):
    data_dir = root_path / la_name / tb_name / 'seq' / 'token-super-tag'
    gold_dataset = _load_data(data_dir, partition, 'gold-super')
    data_vocab = load_data_vocab(root_path / root_path / la_name / tb_name / 'seq' / 'token-super-tag' / 'vocab')
    token_samples = {t: _get_token_samples(gold_dataset[t], data_vocab) for t in gold_dataset}
    max_morphemes = {t: gold_dataset[t].morpheme_id.max() + 1 for t in partition}
    analysis_samples = {t: _get_fixed_analysis_samples(gold_dataset[t], data_vocab, max_morphemes[partition[-1]])
                        for t in gold_dataset}
    return token_samples, analysis_samples, data_vocab


def _save_lattices_vocab(root_path, partition, la_name, tb_name, ma_name):
    lattices_dataset, gold_dataset = load_infused_lattices(root_path, partition, la_name, tb_name, ma_name)
    lattices_vocab = get_data_vocab(lattices_dataset)
    gold_vocab = get_data_vocab(gold_dataset)
    gold_lattices_vocab = get_data_vocabs_union(lattices_vocab, gold_vocab)
    save_data_vocab(root_path / root_path / la_name / tb_name / 'lattice' / ma_name / 'vocab', gold_lattices_vocab)


def _save_gold_vocab(root_path, partition, la_name, tb_name):
    gold_dataset = load_gold(root_path, partition, la_name, tb_name)
    gold_vocab = get_data_vocab(gold_dataset)
    save_data_vocab(root_path / root_path / la_name / tb_name / 'vocab', gold_vocab)


def _save_gold_super_vocab(root_path, partition, la_name, tb_name):
    gold_dataset = load_gold_token_super_tag(root_path, partition, la_name, tb_name)
    gold_vocab = get_data_vocab(gold_dataset)
    save_data_vocab(root_path / root_path / la_name / tb_name / 'seq' / 'token-super-tag' / 'vocab', gold_vocab)


def _save_gold_ft_emb(root_path, ft_root_path, la_name, tb_name):
    vocab_dir_path = root_path / la_name / tb_name / 'vocab'
    data_vocab = load_data_vocab(vocab_dir_path)
    ft_model_path = ft_root_path / f'models/cc.{la_name}.300.bin'
...
def _save_gold_super_ft_emb(root_path, ft_root_path, la_name, tb_name):
    vocab_dir_path = root_path / la_name / tb_name / 'seq' / 'token-super-tag' / 'vocab'
    data_vocab = load_data_vocab(vocab_dir_path)
    ft_model_path = ft_root_path / f'models/cc.{la_name}.300.bin'
    chars_vec_file_path = vocab_dir_path / 'chars.vec'
    tokens_vec_file_path = vocab_dir_path / 'tokens.vec'
...
def main():
    ...
    for la_name in ['tr']:
        ...
        _save_gold_super_vocab(root_path, partition, la_name, tb_name)
        _save_lattices_vocab(root_path, partition, la_name, tb_name, ma_name)
        _save_gold_ft_emb(root_path, ft_path, la_name, tb_name)
        _save_gold_super_ft_emb(root_path, ft_path, la_name, tb_name)
        _save_lattice_ft_emb(root_path, ft_path, la_name, tb_name, ma_name)

        token_samples, morph_samples, data_vocab = load_lattices_data_samples(root_path, partition, la_name, tb_name,
                                                                              ma_name)
        for partition_type in partition:
